    ...
    <div className="space-y-3">
      <UserPageHero
        ...
        statusItems={[
          {
            label: 'Meals',
            value: mealsSelected ? `${mealsCompleted}/${mealsSelected} complete` : 'Choose today',
            tone: mealsDone ? 'good' : 'neutral',
          },
          {
            label: 'Training',
            value: trainingDone ? 'Complete' : trainingReady ? 'Ready' : 'Waiting',
            tone: trainingDone ? 'good' : trainingReady ? 'neutral' : 'warn',
          },
        ]}
      >
        <Link
          href={continueAction.href}
          className="flex min-h-12 items-center justify-between rounded-2xl bg-[var(--color-accent)] px-4 text-sm font-semibold text-[var(--color-text-on-accent)] transition-transform active:scale-[0.99]"
        >
      ...
      <section className="grid grid-cols-2 gap-3" aria-label="Your program">
        ...
        <ProgramCard
          ...
          detail={
            summary.training.activeSessionId
              ? 'Session in progress'
              : trainingReady
                ? 'Your plan is ready'
                : 'Open your program'
      ...
      <section className="rounded-[26px] border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-4">
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm font-semibold text-[var(--color-text)]">
              {dailyTotal > 0 ? `${dailyCompleted} of ${dailyTotal} steps complete` : 'Your program is ready'}
            </p>
            <p className="mt-1 text-xs leading-5 text-[var(--color-text-muted)]">
              {dailyTotal > 0 && dailyCompleted === dailyTotal
                ? 'Everything planned for today is complete.'
                : 'Keep your nutrition and training moving together.'}
            </p>
          </div>
        ...
        {summary.streakCount > 0 ? (
          <p className="mt-3 flex items-center gap-2 border-t border-[var(--color-border)] pt-3 text-xs text-[var(--color-text-muted)]">
            <Check aria-hidden="true" focusable="false" size={14} className="text-[var(--color-accent)]" />{' '}
            {summary.streakCount} day consistency streak
          </p>
        ) : null}
      </section>
      ...
      <Link
        href="/user/learn"
        className="group flex min-h-[88px] items-center gap-4 rounded-[26px] border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 transition-colors hover:border-[var(--color-accent)]/40"
      >
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[var(--color-accent-translucent)] text-[var(--color-accent)]">
          <BookOpenText aria-hidden="true" focusable="false" size={19} />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--color-accent)]">
            Method
          </span>
          <span className="mt-1 block text-sm font-semibold text-[var(--color-text)]">
            Build the knowledge behind your program
          </span>
        </span>
        <ArrowRight
          aria-hidden="true"
...
function ProgramCard({
  ...
  return (
    <Link
      href={href}
      className="flex min-h-40 flex-col rounded-[26px] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 transition-transform active:scale-[0.99]"
    >
      <span className="grid h-9 w-9 place-items-center rounded-2xl bg-[var(--color-bg-alt)] text-[var(--color-accent)]">
        {icon}
      </span>
      ...
      {typeof progress === 'number' ? (
        <div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--color-bg-alt)]">
          <div className="h-full rounded-full bg-[var(--color-accent)]" style={{ width: `${progress}%` }} />
        </div>
      ) : active ? (
        <div className="mt-2 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[0.14em] text-[var(--color-accent)]">
          <CalendarCheck2 aria-hidden="true" focusable="false" size={12} /> Active
        </div>
      ) : null}
    </Link>
  );
