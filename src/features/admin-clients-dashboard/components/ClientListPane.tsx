  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                {clients.length} {viewMode.toLowerCase()} clients
              </p>
            </div>
          </div>
          ...
          {(['ACTIVE', 'INACTIVE', 'ARCHIVED'] as const).map(mode => (
            ...
            >
              <span className="block">
                {mode === 'ACTIVE' ? 'Active' : mode === 'INACTIVE' ? 'Inactive' : 'Archived'}
              </span>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                {client.isSystemTemplate ? (
                  ...
                  >
                    {client.status === 'ACTIVE' ? 'Active' : client.status === 'ARCHIVED' ? 'Archived' : 'Inactive'}
                  </span>
                )}
              </div>
            </button>
        ...
        {clients.length === 0 ? <p className="p-3 text-sm text-muted-foreground">No clients found.</p> : null}
      </div>
    </div>
  );
}
