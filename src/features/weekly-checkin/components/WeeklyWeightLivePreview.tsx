...
function WeightTooltip({
  ...
  return (
    <div className="rounded-xl border border-border bg-background px-3 py-2 text-xs shadow-lg">
      ...
      <p className="mt-1 text-muted-foreground">{value == null ? 'No data' : `${value.toFixed(1)} kg`}</p>
    </div>
  );
}
...
export function WeeklyWeightLivePreview({ lastWeekWeight, currentWeight }: Props) {
  ...
  const deltaColor = isDown ? 'var(--color-accent)' : isUp ? 'var(--color-danger, #ef4444)' : 'var(--color-text-muted)';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-end justify-between text-xs" style={{ color: 'var(--color-text-muted)' }}>
        <span>
          Last week: <strong style={{ color: 'var(--color-text)' }}>{lastWeekWeight.toFixed(1)} kg</strong>
        </span>
        {hasCurrent && (
          <span>
            This week: <strong style={{ color: deltaColor }}>{currentWeight.toFixed(1)} kg</strong>
          </span>
        )}
      </div>
