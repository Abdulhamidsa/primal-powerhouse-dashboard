      ...
      <div className="mx-auto max-w-xl space-y-3">
        <motion.section {...reveal} className="relative overflow-hidden rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
          ...
          <div className="relative">
            <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.22em] text-[var(--color-accent)]"><CircleDot aria-hidden="true" focusable="false" size={13} /> Primal · 1:1</p>
        ...
        <motion.section {...reveal} className="rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
          ...
          <div className="mt-5 grid grid-cols-[1fr_auto_1fr] gap-2">
            ...
            <div className="flex items-center"><ArrowRight aria-hidden="true" focusable="false" size={16} className="text-[var(--color-accent)]" /></div>
        ...
        <motion.section {...reveal} className="rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
          ...
          <p className="mt-5 flex items-center gap-2 border-t border-[var(--color-border)] pt-4 text-xs font-semibold text-[var(--color-text-muted)]"><Check aria-hidden="true" focusable="false" size={15} className="text-[var(--color-accent)]" /> One request. A personal follow-up.</p>
        </motion.section>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="mt-2 space-y-2">
        {items.slice(0, 5).map(item => <p key={item} className={`flex gap-1.5 text-[10px] leading-4 ${muted ? 'text-[var(--color-text-muted)]' : 'text-[var(--color-text)]'}`}><Check aria-hidden="true" focusable="false" size={11} className="mt-0.5 shrink-0 text-[var(--color-accent)]" />{item}</p>)}
      </div>
    </div>
  );
}
