...
export default function AddClientModal({ isOpen, onClose, onClientAdded }: AddClientModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.currentWeight || isNaN(Number(formData.currentWeight))) newErrors.currentWeight = "Valid current weight required";
    if (!formData.targetWeight || isNaN(Number(formData.targetWeight))) newErrors.targetWeight = "Valid target weight required";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const clientData = {
        ...
        avatar: formData.avatar || `https://images.unsplash.com/photo-${Math.random() > 0.5 ? "1494790108755-2616b612b890" : "1507003211169-0a1dd7228f2d"}?w=150`,
        status: "ACTIVE",
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <label key={level.value} className={`relative p-4 border rounded-lg cursor-pointer transition-all ${formData.activityLevel === level.value ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-gray-400"}`}>
                  <input type="radio" name="activityLevel" value={level.value} checked={formData.activityLevel === level.value} onChange={(e) => handleInputChange("activityLevel", e.target.value)} className="sr-only" />
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={() => addArrayItem("goals")} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-sm">
                + Add Goal
              </button>
            </div>
            <div className="space-y-2">
              {formData.goals.map((goal, index) => (
                <div key={index} className="flex gap-2">
                  <select value={goal} onChange={(e) => handleArrayChange("goals", index, e.target.value)} className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                  ...
                  {formData.goals.length > 1 && (
                    <button type="button" onClick={() => removeArrayItem("goals", index)} className="px-3 py-3 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                      ✕
                    </button>
                  )}
                </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={() => addArrayItem("dietaryRestrictions")} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-sm">
                + Add Restriction
              </button>
            </div>
            <div className="space-y-2">
              {formData.dietaryRestrictions.map((restriction, index) => (
                <div key={index} className="flex gap-2">
                  <select value={restriction} onChange={(e) => handleArrayChange("dietaryRestrictions", index, e.target.value)} className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                  ...
                  {formData.dietaryRestrictions.length > 1 && (
                    <button type="button" onClick={() => removeArrayItem("dietaryRestrictions", index)} className="px-3 py-3 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                      ✕
                    </button>
                  )}
                </div>
