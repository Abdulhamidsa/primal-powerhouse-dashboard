...
const enhancedSampleMeals: Meal[] = [
  {
    ...
    type: "breakfast",
    description: "A nutrient-packed breakfast featuring creamy avocado on artisanal whole grain bread, topped with perfectly poached eggs and Mediterranean herbs.",
    calories: 420,
    protein: 18,
    carbs: 28,
    fat: 24,
  ...
  },
  {
    ...
    type: "lunch",
    description: "A protein-packed twist on the classic Caesar salad, featuring perfectly grilled chicken breast over crisp romaine lettuce with a lighter homemade Caesar dressing.",
    calories: 380,
    protein: 35,
    carbs: 12,
    fat: 22,
  ...
  },
  {
    ...
    type: "dinner",
    description: "Premium Atlantic salmon with a fragrant herb crust, served alongside fluffy quinoa pilaf with roasted vegetables and a lemon-tahini drizzle.",
    calories: 520,
    protein: 42,
    carbs: 45,
    fat: 18,
    ...
    cholesterol: 78,
    ingredients: [
      { id: "1", name: "Atlantic salmon fillet", amount: 6, unit: "oz", notes: "skin-on, pin bones removed" },
      { id: "2", name: "Quinoa", amount: 1 / 2, unit: "cup", notes: "tri-color preferred" },
      { id: "3", name: "Mixed vegetables", amount: 1, unit: "cup", notes: "bell peppers, zucchini, carrots" },
    ...
    instructions: [
      { id: "1", step: 1, instruction: "Preheat oven to 400°F. Rinse quinoa and cook according to package instructions with vegetable broth for extra flavor.", timeEstimate: 15, temperature: "400°F" },
      { id: "2", step: 2, instruction: "Dice vegetables into uniform pieces and toss with half the olive oil, salt, and pepper. Roast for 15 minutes.", timeEstimate: 20 },
      { id: "3", step: 3, instruction: "Mix fresh herbs with panko breadcrumbs, lemon zest, and a pinch of salt to create the herb crust.", timeEstimate: 3 },
      { id: "4", step: 4, instruction: "Pat salmon dry, season with salt and pepper. Press herb mixture onto the flesh side of the salmon.", timeEstimate: 3 },
      { id: "5", step: 5, instruction: "Heat an oven-safe skillet over medium-high heat. Sear salmon skin-side up for 3 minutes, then flip and transfer to oven for 8-10 minutes.", timeEstimate: 12 },
      { id: "6", step: 6, instruction: "Whisk tahini with lemon juice and a splash of water. Fluff quinoa and mix with roasted vegetables. Serve salmon over quinoa with tahini drizzle.", timeEstimate: 3 },
    ],
    prepTime: 15,
    cookTime: 25,
    servings: 1,
    tags: ["high-protein", "omega-3", "gluten-free", "heart-healthy", "anti-inflammatory"],
    images: ["https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=800", "https://images.unsplash.com/photo-1519708227418-c8fd9a32b7a2?w=800", "https://images.unsplash.com/photo-1565299507177-b0ac66763828?w=800"],
    difficulty: "medium",
    equipment: ["Oven-safe skillet", "Medium saucepan", "Baking sheet", "Whisk"],
    tips: ["Let salmon come to room temperature before cooking", "Use a meat thermometer - internal temp should reach 145°F", "Toast quinoa in dry pan before adding liquid for nuttier flavor", "Don't overcook - salmon should flake easily but still be moist"],
    nutritionNotes: "Excellent source of omega-3 fatty acids, complete protein, and complex carbohydrates. Anti-inflammatory and heart-healthy.",
  ...
  },
  {
    ...
    type: "snack",
    description: "Creamy Greek yogurt layered with fresh seasonal berries and crunchy homemade granola, drizzled with raw honey and topped with chopped almonds.",
    calories: 220,
    protein: 15,
    carbs: 30,
    fat: 4,
    ...
    ingredients: [
      ...
      { id: "2", name: "Mixed berries", amount: 1 / 2, unit: "cup", notes: "strawberries, blueberries, raspberries" },
    ...
    instructions: [
      { id: "1", step: 1, instruction: "Wash and prepare fresh berries. Hull strawberries and cut into bite-sized pieces if large.", timeEstimate: 3 },
      { id: "2", step: 2, instruction: "Mix Greek yogurt with vanilla extract and a tiny pinch of lemon zest if using.", timeEstimate: 1 },
      { id: "3", step: 3, instruction: "In a glass or bowl, layer half the yogurt mixture at the bottom.", timeEstimate: 1 },
      { id: "4", step: 4, instruction: "Add half the berries and half the granola over the yogurt layer.", timeEstimate: 1 },
      { id: "5", step: 5, instruction: "Repeat layers with remaining yogurt, berries, and granola.", timeEstimate: 1 },
      { id: "6", step: 6, instruction: "Drizzle with honey and top with toasted sliced almonds. Serve immediately.", timeEstimate: 1 },
    ],
    prepTime: 8,
    cookTime: 0,
    servings: 1,
    tags: ["vegetarian", "high-protein", "antioxidants", "quick", "probiotic", "gluten-free"],
    images: ["https://images.unsplash.com/photo-1488477181946-6428a0291777?w=800", "https://images.unsplash.com/photo-1571091718767-18b5b1457add?w=800"],
    difficulty: "easy",
    equipment: ["Glass or bowl", "Measuring cups"],
    tips: ["Use full-fat Greek yogurt for better satiety", "Prepare granola in batches for meal prep", "Layer in clear glass to showcase the beautiful colors", "Add berries just before serving to prevent soggy granola"],
    nutritionNotes: "High in probiotics for gut health, antioxidants from berries, and provides sustained energy from protein and healthy fats.",
...
export default function AssignMealsPage() {
  ...
  const filteredMeals = enhancedSampleMeals.filter((meal) => {
    ...
    const matchesRestrictions = !client.dietaryRestrictions.some((restriction) => meal.tags.some((tag) => tag.toLowerCase().includes(restriction.toLowerCase())) === false);
              ...
              <div className="p-6 border-b border-gray-200">
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                  <div className="flex-1">
                    <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search meals..." className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                  </div>
                  <select value={filterType} onChange={(e) => setFilterType(e.target.value as MealType | "all")} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div key={meal.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <div className="flex gap-4">
                        ...
                        <div className="w-24 h-24 rounded-lg overflow-hidden flex-shrink-0">
                          {meal.images[0] ? <img src={meal.images[0]} alt={meal.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-100 flex items-center justify-center text-2xl">{getMealTypeIcon(meal.type)}</div>}
                        </div>
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getMealTypeColor(meal.type)}`}>
                                  {getMealTypeIcon(meal.type)} {meal.type}
                                </span>
                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getDifficultyColor(meal.difficulty)}`}>{meal.difficulty}</span>
                            ...
                            {/* Assign Button */}
                            <button onClick={() => assignMeal(meal, selectedDate, selectedMealType)} className="ml-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
                    ...
                    <div key={date} className="border border-gray-200 rounded-lg p-3">
                      ...
                      <div className="grid grid-cols-2 gap-1 text-xs">
                        {(["breakfast", "lunch", "dinner", "snack"] as MealType[]).map((mealType) => {
                          ...
                          return (
                            <div key={mealType} className={`p-2 rounded border ${assignedMeal ? "bg-green-50 border-green-200 text-green-700" : "bg-gray-50 border-gray-200 text-gray-400"}`}>
                              <div className="flex items-center justify-between">
                                <span>{getMealTypeIcon(mealType)}</span>
                                {assignedMeal && (
                                  <button onClick={() => removeMeal(date, mealType)} className="text-red-500 hover:text-red-700">
                                    ✕
                                  </button>
                                )}
                              </div>
                              {assignedMeal && <div className="mt-1 font-medium truncate">{assignedMeal.meal.name}</div>}
                            </div>
                          );
                        })}
                      </div>
