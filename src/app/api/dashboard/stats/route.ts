...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgRating = workoutsWithRating.length > 0 ? workoutsWithRating.reduce((sum, w) => sum + (w.rating || 0), 0) / workoutsWithRating.length : 0;

    // Calculate growth percentages
    const clientGrowth = lastMonthClients > 0 ? ((thisMonthClients - lastMonthClients) / lastMonthClients) * 100 : thisMonthClients > 0 ? 100 : 0;

    const sessionGrowth = lastMonthWorkouts > 0 ? ((thisMonthWorkouts - lastMonthWorkouts) / lastMonthWorkouts) * 100 : thisMonthWorkouts > 0 ? 100 : 0;
