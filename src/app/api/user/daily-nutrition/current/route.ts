...
import { dayDateKeySchema, upsertDailyNutritionSchema } from '@/features/daily-nutrition/schemas/dailyNutrition.schema';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const entry = await (prisma as any).dailyNutritionLog.findUnique({
      where: {
        clientId_dayDate: {
    ...
    console.error('[DAILY_NUTRITION_GET] Failed:', error);
    return jsonWithCache({ error: 'Failed to load daily nutrition status' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const parsed = upsertDailyNutritionSchema.safeParse(await request.json());
    if (!parsed.success) {
      return jsonWithCache({ error: 'Invalid daily nutrition payload' }, { status: 400 });
    }

    const { dayDate, status, note } = parsed.data;
    const dayDateUtc = parseDateKeyUtc(dayDate);

    const entry = await (prisma as any).dailyNutritionLog.upsert({
      where: {
        clientId_dayDate: {
          clientId: user.userId,
          dayDate: dayDateUtc,
    ...
    console.error('[DAILY_NUTRITION_PUT] Failed:', error);
    ...
    if (message.includes('daily_nutrition_logs') || message.includes('does not exist') || message.includes('P2021')) {
      return jsonWithCache({ error: 'Daily nutrition storage is not initialized yet' }, { status: 500 });
    }

    return jsonWithCache({ error: 'Failed to save daily nutrition status' }, { status: 500 });
  }
}
