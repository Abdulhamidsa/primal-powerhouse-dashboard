...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string; checkInId: string }> }) {
  try {
    ...
    if (!checkIn || checkIn.clientId !== clientId || checkIn.client.coachId !== actor.id) {
      return jsonWithCache({ error: 'Weekly check-in not found for this coach' }, { status: 404 });
    }
    ...
    console.error('[ADMIN_WEEKLY_CHECKIN_REVIEW_POST] Failed:', error);
    return jsonWithCache({ error: 'Failed to mark weekly check-in as reviewed' }, { status: 500 });
  }
}
