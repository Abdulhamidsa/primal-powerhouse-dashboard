  ...
  return (
    <Screen title="Privacy & data" onRefresh={m.query.refresh} refreshing={m.query.isValidating}>
      ...
      {m.query.data ? (
        <>
            ...
            <Card>
              ...
              {m.query.data.exportJobs.map(job => (
                <Copy key={job.id} muted>
                  ...
                  {job.downloadCount ? ` · downloaded ${job.downloadCount} time${job.downloadCount === 1 ? '' : 's'}` : ''}
                </Copy>
              ))}
            </Card>
          ) : null}
          <Card>
            ...
            {m.query.data.activeSession.issuedAt ? <Copy muted>Started · {new Date(m.query.data.activeSession.issuedAt).toLocaleString()}</Copy> : null}
            {m.query.data.activeSession.expiresAt ? <Copy muted>Access refresh due · {new Date(m.query.data.activeSession.expiresAt).toLocaleString()}</Copy> : null}
          </Card>
        </>
      ) : null}
      <Card>
        ...
        <Button title="Export and share my data" onPress={m.export} disabled={m.offline || m.action.pending || !m.password} />
      </Card>
      <Card>
      ...
      <Card>
        ...
        <Copy>This will deactivate your account and start the existing deletion process. Enter your password above and type DELETE MY ACCOUNT to confirm.</Copy>
        {m.query.data?.activeDeletionRequest ? (
          <>
            ...
            <Copy muted>Scheduled deletion · {new Date(m.query.data.activeDeletionRequest.scheduledHardDeleteAt).toLocaleDateString()}</Copy>
          </>
        ) : (
          <>
            <Field label="Confirmation" value={m.confirmation} onChange={m.setConfirmation} />
            <Button secondary title="Delete my account" onPress={m.delete} disabled={m.offline || m.action.pending || !m.password || m.confirmation !== 'DELETE MY ACCOUNT'} />
          </>
        )}
      </Card>
    </Screen>
